...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    for script in scripts:
        if "application/ld+json" in script.get("type"):
            ...
            pycon = json.loads(script.text.strip())
            name = pycon["name"]
            city = pycon["location"]["address"]["addressLocality"]
            country = pycon["location"]["address"]["addressCountry"]
            start_date = datetime.strptime(pycon["startDate"], "%Y-%m-%d")
            end_date = datetime.strptime(pycon["endDate"], "%Y-%m-%d")
            url = pycon["url"]
            if start_date.year in (2018, 2019) and "pycon" in name.lower():
                result.append(
                    PyCon(
                        name=name.strip(),
                        city=city.strip(),
...
def filter_pycons(pycons: List[PyCon],
    ...
    result = []
    for pycon in pycons:
        if get_continent(pycon.country) == continent and pycon.start_date.year == year:
            result.append(pycon)
